...
public class MenuRepositoryJdbc implements MenuRepository {
	...
	public void add(Menu menu) {
		jdbcTemplate.update(
				"INSERT menu (id,label,path,`level`,`order`,`url`,`type`,`style`,`disabled`) VALUES (?,?,?,?,?,?,?,?,?)",
				menu.getId(), menu.getLabel(), menu.getPath(), menu.getLevel(), menu.getOrder(), menu.getUrl(),
				menu.getType(), menu.getStyle(), menu.isDisabled() ? 1 : 0);
	}

	@Override
	public void update(Menu menu) {
		jdbcTemplate.update("update menu SET label=?,`order`=?,url=?,disabled=?,`type`=?,`style`=? WHERE id=?",
				menu.getLabel(), menu.getOrder(), menu.getUrl(), menu.isDisabled() ? 1 : 0, menu.getType(),
	...
	public List<Menu> roleMenus(String roleId) {
		return jdbcTemplate.query("select m.* from menu m join role_menu rm on m.id=rm.menu_id where rm.role_id=?",
				createMapper(), roleId);
	}
	...
	public List<Menu> getNavMenus(String userId) {
		return jdbcTemplate.query(
				"select m.* from menu m join role_menu rm on m.id=rm.menu_id join user_role ur on rm.role_id=ur.role_id where m.disabled=0 and ur.uid=?",
				createMapper(), userId);
	}
}
