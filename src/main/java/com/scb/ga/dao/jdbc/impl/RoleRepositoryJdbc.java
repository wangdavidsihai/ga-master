...
public class RoleRepositoryJdbc implements RoleRepository {
	...
	public void add(Role role) {
		jdbcTemplate.update("INSERT role (id,`name`,disabled,description) VALUES (?,?,?,?)", role.getId(),
				role.getName(), role.isDisabled() ? 1 : 0, role.getDescription());
	}

	@Override
	public void update(Role role) {
		jdbcTemplate.update("update role SET `name` =?,disabled=?,description=? WHERE id=?", role.getName(),
				role.isDisabled() ? 1 : 0, role.getDescription(), role.getId());
	}
	...
	public void updateMenus(String rid, List<String> mids) {
		...
		if (!CollectionUtils.isEmpty(mids)) {
			jdbcTemplate.batchUpdate("INSERT role_menu (role_id,menu_id) VALUES (?,?)",
	...
	public void updateResources(String rid, List<String> resources) {
		...
		if (!CollectionUtils.isEmpty(resources)) {
			jdbcTemplate.batchUpdate("INSERT role_resource (role_id,resource_id) VALUES (?,?)",
					new BatchPreparedStatementSetter() {
						@Override
						public void setValues(PreparedStatement ps, int i) throws SQLException {
							ps.setString(1, rid);
	...
	public boolean contains(String roleName) {
		return jdbcTemplate.queryForObject("select count(`name`) from role where `name`=?", Integer.class,
				roleName) > 0;
	}
	...
	public List<Role> getRoles(String userId) {
		return jdbcTemplate.query("select * from role r join user_role ur on r.id=ur.role_id where ur.uid=?",
