...
public class UserRepositoryJdbc implements UserRepository {
	...
	public void add(User user) {
		jdbcTemplate.update("INSERT user (id,username,password,email,disabled,createTime,salt) VALUES (?,?,?,?,?,?,?)",
				user.getId(), user.getUsername(), user.getPassword(), user.getEmail(), user.isDisabled() ? 1 : 0,
				new Date(), user.getSalt());
	}

	@Override
	public void update(User user) {
		jdbcTemplate.update("UPDATE user SET username=?,email=?,password=? WHERE id=?", user.getUsername(),
	...
	public List<User> list() {
		return jdbcTemplate.query("select * from user where username <> 'root'",
	...
	public boolean hasResourcePermission(String uid, String resourceCode) {
		return jdbcTemplate.query(
				"select count(*) from user_role ur join role_resource rr on ur.role_id=rr.role_id where ur.uid=? and rr.resource_id=?",
	...
	public List<User> getUserByUname(String username) {
		return jdbcTemplate.query("select * from user where username =?", BeanPropertyRowMapper.newInstance(User.class),
				username);
	}

	@Override
	public List<User> getUserByEmail(String email) {
		return jdbcTemplate.query("select * from user  where email =?", BeanPropertyRowMapper.newInstance(User.class),
				email);
	}
