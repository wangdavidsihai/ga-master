...
import com.scb.ga.interceptor.UrlSecurityInterceptor;
import com.scb.ga.security.UserDetailService;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Value("${remember.key}")
	private String key = "wsh_8818@@hotmail.com";
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.sessionManagement().invalidSessionUrl("/to-login");

		// http.rememberMe().userDetailsService(userDetailService).key(key).useSecureCookie(false).alwaysRemember(true);
		http.addFilterAt(urlSecurityInterceptor(), FilterSecurityInterceptor.class);// customized interceptor for
																					// security processing
		// http.authorizeRequests() mapping to FilterSecurityInterceptor，no config no
		// FilterSecurityInterceptor
		http.formLogin().loginProcessingUrl("/login").loginPage("/to-login").defaultSuccessUrl("/")
	...
	public void configure(WebSecurity web) throws Exception {
		web.ignoring().antMatchers("/**/*.js", "/**/*.js.map", "/**/*.ts", "/**/*.css", "/**/*.css.map", "/**/*.png",
				"/**/*.gif", "/**/*.jpg", "/**/*.jpeg", "/**/*.fco", "/**/*.woff", "/**/*.woff2", "/**/*.font",
				"/**/*.svg", "/**/*.ttf", "/**/*.pdf", "/*.ico", "/admin/api/**", "/404", "/401", "/403", "/error");
	}
